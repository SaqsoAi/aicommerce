...
export default function ProductCatalogClient() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-zinc-50 text-zinc-950 dark:bg-black dark:text-white">
    ...
    {filterOpen ? <div className="fixed inset-0 z-[100] lg:hidden" data-saqso-filter-drawer data-open={filterOpen ? "true" : "false"}><button type="button" aria-label="Close filters" onClick={()=>setFilterOpen(false)} className="absolute inset-0 bg-black/65 backdrop-blur-[2px]"/><aside role="dialog" aria-modal="true" aria-label="Product filters" className="absolute inset-x-0 bottom-0 flex h-auto max-h-[82dvh] flex-col overflow-hidden rounded-t-[1.5rem] bg-white text-zinc-950 shadow-2xl dark:bg-zinc-950 dark:text-white"><div className="flex shrink-0 items-center justify-between border-b border-zinc-200 bg-white px-4 py-3 dark:border-white/10 dark:bg-zinc-950"><div><h2 className="text-lg font-black">Filters</h2><p className="text-xs text-zinc-500">{activeFilterCount ? `${activeFilterCount} selected` : "Choose what you need"}</p></div><button type="button" onClick={()=>setFilterOpen(false)} aria-label="Close filters" className="grid h-11 w-11 place-items-center rounded-full border border-zinc-300 bg-white text-black dark:border-white/20 dark:bg-black dark:text-white"><X size={18}/></button></div><div className="max-h-[58dvh] overflow-y-auto overscroll-contain px-3 py-2"><FilterPanel {...filterProps}/></div><div className="grid shrink-0 grid-cols-[auto_1fr] gap-2 border-t border-zinc-200 bg-white p-3 pb-[calc(.75rem+env(safe-area-inset-bottom))] dark:border-white/10 dark:bg-zinc-950">{activeFilterCount ? <button type="button" onClick={clearFilters} className="min-h-12 rounded-xl border border-zinc-300 px-4 text-sm font-bold dark:border-white/20">Clear</button> : null}<button type="button" onClick={()=>setFilterOpen(false)} className="min-h-12 rounded-xl bg-zinc-950 px-5 text-sm font-black text-white dark:bg-white dark:text-black">Show {products.length} products</button></div></aside></div> : null}
  </main>;
}
