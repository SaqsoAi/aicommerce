    ...
    <div className="space-y-5">
      <section className="rounded-[1.5rem] border border-white/10 bg-[#111114] p-5 text-white shadow-[0_18px_70px_rgba(0,0,0,0.28)]">
        ...
        <div className="mt-5 space-y-3">
          {loading ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white/50">Loading orders...</div>
          ) : orders.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white/50">No real orders found yet.</div>
          ) : orders.map((order, i) => (
            <Link
              key={order.id || i}
              href={`/orders/${order.id || ""}`}
              className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-[#18181b] p-4 transition hover:border-rose-300/50"
            >
      ...
      <section className="rounded-[1.5rem] border border-white/10 bg-[#111114] p-5 text-white shadow-[0_18px_70px_rgba(0,0,0,0.28)]">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {loading ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white/50 sm:col-span-2 lg:col-span-4">Loading wishlist...</div>
          ) : wishlist.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white/50 sm:col-span-2 lg:col-span-4">No real wishlist items found yet.</div>
          ) : wishlist.map((item, i) => (
            <Link
              key={item.id || item.productId || i}
              href={item.href || `/product/${item.productId || item.product?.id || item.id || ""}`}
              className="overflow-hidden rounded-2xl border border-white/10 bg-[#18181b] transition hover:border-rose-300/50"
            >
      ...
      <section className="rounded-[1.5rem] border border-white/10 bg-[#111114] p-5 text-white shadow-[0_18px_70px_rgba(0,0,0,0.28)]">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {loading ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white/50 sm:col-span-2 lg:col-span-3">Loading recommendations...</div>
          ) : recommended.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white/50 sm:col-span-2 lg:col-span-3">No real recommendations found yet.</div>
