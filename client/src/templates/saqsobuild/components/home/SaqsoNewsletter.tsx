export default function SaqsoNewsletter({ settings = {} }: { settings?: Record<string, unknown> }) {
  if (settings.enabled === false) return null;
  return (
    <section className="mx-auto max-w-7xl px-6 py-12">
      <div className="rounded-[2rem] bg-zinc-100 p-8 dark:bg-zinc-900">
        <h2 className="text-4xl font-black">{String(settings.title || "Get store updates")}</h2>
        <p className="mt-3 text-zinc-500">{String(settings.subtitle || "Receive new arrivals and relevant offers.")}</p>
