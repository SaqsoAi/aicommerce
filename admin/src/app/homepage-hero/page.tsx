...
export default function HomepageHeroManagerPage() {
  ...
  return (
    <div className="min-h-0 w-full min-w-0 bg-[#060913] text-white">
      <div className="mx-auto max-w-[1800px] p-4 sm:p-5 lg:p-6">
        <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={loadHeroes} className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-bold hover:bg-white/10">Refresh</button>
        ...
        <div className="grid gap-5 2xl:grid-cols-[1fr_430px]">
          <div className="min-w-0 space-y-5">
            <section className="grid gap-4 md:grid-cols-4">
              {[
                ...
                <div key={String(label)} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 shadow-xl shadow-black/20">
            ...
            <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
              <div className="grid gap-3 lg:grid-cols-[1fr_170px_170px_170px]">
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search hero by title, subtitle..." className="rounded-xl border border-white/10 bg-[#0b111c] px-4 py-3 text-sm outline-none focus:border-violet-400" />
                <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-xl border border-white/10 bg-[#0b111c] px-4 py-3 text-sm outline-none">
                  <option value="all">All Status</option>
                  <option value="live">Live</option>
                  <option value="draft">Draft</option>
                </select>
                <select value={type} onChange={(e) => setType(e.target.value)} className="rounded-xl border border-white/10 bg-[#0b111c] px-4 py-3 text-sm outline-none">
                  <option value="all">All Types</option>
                  <option value="image">Image</option>
                  <option value="video">Video</option>
                </select>
                <select value={sort} onChange={(e) => setSort(e.target.value)} className="rounded-xl border border-white/10 bg-[#0b111c] px-4 py-3 text-sm outline-none">
            ...
            {message && <div className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-3 text-sm text-amber-200">{message}</div>}
                    ...
                    {filtered.map((hero) => {
                      ...
                      return (
                        <tr key={String(hero.id || hero.headline)} onClick={() => setSelectedId(String(hero.id || ""))} className={`cursor-pointer border-b border-white/10 transition hover:bg-white/[0.05] ${isSelected ? "bg-violet-500/10" : ""}`}>
                          ...
                          <td className="px-4 py-3">
                            <button onClick={(e) => { e.stopPropagation(); toggleHero(hero); }} className={`rounded-lg px-3 py-1 text-xs font-black ${hero.active ? "bg-emerald-500/15 text-emerald-300" : "bg-slate-500/20 text-slate-300"}`}>{hero.active ? "Live" : "Draft"}</button>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              ...
                              <button onClick={(e) => { e.stopPropagation(); duplicateHero(hero); }} className="rounded-lg bg-white/10 px-3 py-2 text-xs font-bold text-slate-200 hover:bg-white/20">Copy</button>
                              <button onClick={(e) => { e.stopPropagation(); deleteHero(hero); }} className="rounded-lg bg-red-500/15 p-2 text-red-300 hover:bg-red-500/25"><Icon name="trash" /></button>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          <aside className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/20">
            ...
            <dl className="mt-5 grid grid-cols-[110px_1fr] gap-3 text-sm">
