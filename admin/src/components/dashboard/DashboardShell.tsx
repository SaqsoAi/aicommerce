...
function EmptyState({ label }: { label: string; source?: string }) {
  return (
...
function Rows({ rows, fallback, source }: { rows: Array<[string, string, DataState?, string?]>; fallback: string; source: string }) {
  ...
  return <div className="ds-rows">{rows.map(([a, b, state]) => <div key={a} className={state === "unavailable" ? "is-unavailable" : ""}><span>{a}</span><b>{b}</b></div>)}</div>;
}
  ...
  return (
    <>
      ...
      <section className="ds-commerce-content">
        ...
        {data.role === "ADMIN" ? <Panel title="Top Customers" action={<button onClick={() => onAction("users")}>View All</button>}>{data.customers.length ? <div className="ds-products">{data.customers.map(([name, orders, total]) => <article key={name}><i /><div><b>{name}</b><span>{orders}</span></div><strong>{total}</strong></article>)}</div> : <EmptyState label="No customer ranking rows were returned." source="/api/dashboard/summary" />}</Panel> : null}
      </section>
  ...
  if (data.mode === "super") {
    return (
      <aside className="ds-right">
        ...
        <Panel title="System Monitor" action={<button onClick={() => onAction("performance")}>View All</button>}><Rows rows={data.system} fallback="Runtime telemetry is unavailable." source="/api/dashboard/super-admin" /></Panel>
  ...
  return (
    <aside className="ds-right">
      <Panel title="Store Status" action={<button onClick={() => onAction("report")}>Online</button>}>
        <Rows rows={data.store} fallback="Store status unavailable." source="/api/dashboard/summary" />
      </Panel>
