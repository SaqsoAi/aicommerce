...
export default function AiCapabilityDiscoveryPanel({imported}:{imported:string[]}){const plans=buildAiCapabilityPlan(imported);return <section style={{marginTop:16,padding:18,border:"1px solid rgba(148,163,184,.18)",borderRadius:14,background:"rgba(15,23,42,.72)"}}><h2>AI Capability Reconciliation</h2><p style={{color:"#94a3b8"}}>Imported AI signals are reconciled with the existing gateway, provider, model and permission architecture. Existing capabilities are reused; only genuine gaps receive a new implementation plan.</p><div style={{display:"grid",gap:10}}>{plans.map(p=><div key={p.name} style={{padding:12,border:"1px solid rgba(148,163,184,.14)",borderRadius:10}}><div style={{display:"flex",justifyContent:"space-between",gap:12}}><b>{p.name}</b><span>{p.status}</span></div><p style={{margin:"6px 0 0",color:"#a8b7cc"}}>{p.plan}</p></div>)}</div></section>}
