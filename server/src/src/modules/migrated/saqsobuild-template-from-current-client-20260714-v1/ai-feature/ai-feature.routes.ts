...
ai_featureMigratedRouter.get("/", async (_req, res) => {
  res.status(501).json({ success: false, code: "MIGRATED_FEATURE_NOT_BOUND", message: "Bind this generated contract to the existing service or approved database implementation before publish.", templateKey: "saqsobuild-template-from-current-client-20260714-v1" });
});
